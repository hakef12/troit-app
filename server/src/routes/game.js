...
router.post('/score', requireAuth, (req, res) => {
  ...
  if (awarded > 0) {
    db.prepare('UPDATE users SET points = points + ?, game_points_today = ?, game_points_date = ? WHERE id = ?').run(
      awarded,
      pointsToday + awarded,
      today(),
      req.userId
  ...
  const updated = db.prepare('SELECT points FROM users WHERE id = ?').get(req.userId);
